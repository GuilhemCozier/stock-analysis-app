...
1. **POST /api/sector/start** - Start sector analysis
...
2. **GET /api/analysis/list** - List all analyses for sidebar
   - Fetch all SectorAnalysis (and future StockAnalysis) records
   - Include type field to distinguish sector vs stock
   - Return: { success, data: [...] }

3. **GET /api/sector/[id]** - Get analysis status and data
...
4. **GET /api/sector/[id]/stream** - SSE endpoint for real-time updates
   - Stream Server-Sent Events (SSE)
   - Query JobStatus for related jobs every 1-2 seconds
   - Send progress updates: { type, jobId, status, progress, data }
   - Handle connection cleanup

5. **POST /api/subsector/[id]/approve** - Approve sub-sector for deep analysis
...
6. **GET /api/subsector/[id]/stocks** - Get stocks in sub-sector
   - Fetch SubSector with all stocks and their analyses
   - Include deepAnalysis with insights if available
   - Return ranked list

7. **GET /api/stock/list** - List all stocks with completed analyses
   - Fetch all stocks with completed StockAnalysis
   - Return data shaped for DatabaseTable component
   - Support filtering by sectorId query param

8. **GET /api/stock/[id]/analysis** - Get deep analysis for stock
   - Fetch Stock with StockAnalysis and insights
   - Return null if no analysis exists
   - Include formatted insights JSON

9. **POST /api/stock/[id]/reanalyze** - Trigger analysis for non-top-5 stock
...
**File Structure:**
- /src/app/api/analysis/list/route.ts
- /src/app/api/sector/start/route.ts
- /src/app/api/sector/[id]/route.ts
- /src/app/api/sector/[id]/stream/route.ts
- /src/app/api/subsector/[id]/approve/route.ts
- /src/app/api/subsector/[id]/stocks/route.ts
- /src/app/api/stock/list/route.ts
